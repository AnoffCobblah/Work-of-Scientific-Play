...
# July 2020: Play and Work Terms in 19th-Century Popular Science

This script combines my Word Flagging and KWIC (tokenizer script) methods in order to create an interactive illustration of the frequency with which the lemma "amus", "plai", "play", "player", "recreat","labor","labori", "labour", "toil", "work", and "worker" were referenced in 19th-century Popular Science. The goal is to determine whether references to work and play make up a larger proportion of the corpus at the end of the century than at the beginning, and to visualize this in such a way that scrolling over a point automatically produces a key words in context (randomly).
...
```{r,  eval=FALSE}
    ...
    PopScidoclocation <- paste0(PopScilocation,"/Corpora/PopSci")
    ...
    PopScistemsearchedtermlist <- c(AmusementTerms,PlayTerms,RecreationTerms,LaborTerms,ToilTerms,WorkTerms)
          ...
          for (j in 1:length(stemltoken)) {
              if (PopScistemsearchedterm == stemltoken[j]) {
                ...
                #Determining Category
                  if(PopScistemsearchedterm %in% AmusementTerms) {temprow[1,9] <- "Amusement"}
                  if(PopScistemsearchedterm %in% PlayTerms) {temprow[1,9] <- "Play"}
                  if(PopScistemsearchedterm %in% RecreationTerms) {temprow[1,9] <- "Recreation"}
                  if(PopScistemsearchedterm %in% LaborTerms) {temprow[1,9] <- "Labor"}
                  if(PopScistemsearchedterm %in% ToilTerms) {temprow[1,9] <- "Toil"}
                  if(PopScistemsearchedterm %in% WorkTerms) {temprow[1,9] <- "Work"}
...
We can then add up the values in SciLifeWordFlagdf to make a table of the frequency of play and work terms for each text: PopSciFreqmat.Again, it's important to do it this way because it lets us assign a random KWIC for later.
...
```{r,  eval=FALSE}
  # Adding values from PopSciWordFlagdf together to get a matrix of normalized frequencies for each category, as PopSciFreqmat
    ...
    for (z in 1:length(Categories)) {   
      tempdf <- PopSciWordFlagdf[grep(Categories[z],PopSciWordFlagdf$Category),]
      
      for (i in 1:length(files)) {
        TempTextName <- gsub(paste0(PopScidoclocation,"/"),"",files[i]) #This grabs just the end of the file path.
        TempTextName <- gsub(".txt","",TempTextName) #This removes the .txt from the end of the name.
        tempdf2 <- tempdf[grep(TempTextName,tempdf$Text),]
        TempDate <- strsplit(TempTextName,"_")[[1]][2]
        TempLength <- tempdf2$Total_Lemma[1]
...
Once we've verified that the data within texts looks right, we can do the same, but per decade, by creating a new dataframe: PopSciFreqDecadedf.

```{r,  eval=FALSE}
  # Adding values from PopSciFreqdf together to get a matrix of normalized frequencies for each decade, as PopSciFreqDecadedf

      PopSciFreqDecadeMat <- matrix(,ncol=6,nrow=1)
      Decades <- c(180, 181, 182, 183, 184, 185, 186, 187, 188, 189)
    for (z in 1:length(Categories)) { 
      tempdf <- PopSciFreqdf[grep(Categories[z],PopSciFreqdf$Category),]
      ...
      for (i in 1:length(Decades)) {
        ...
        PopSciFreqDecadeMat <- rbind(PopSciFreqDecadeMat,temprows)
      }
    }
    PopSciFreqDecadeMat <- PopSciFreqDecadeMat[-1,]
    PopSciFreqDecadedf <- as.data.frame(PopSciFreqDecadeMat)
    PopSciFreqDecadedf
...
#set a category.
z=6
tempdf <- PopSciFreqDecadedf[grep(Categories[z],PopSciFreqDecadedf$Category),]
