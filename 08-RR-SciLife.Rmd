...
# July 2020: Play and Work Terms in 19th-Century Scientific Life Writing
...
This script combines my Word Flagging and KWIC (tokenizer script) methods in order to create an interactive illustration of the frequency with which the lemma "amus", "plai", "play", "player", "recreat","labor","labori", "labour", "toil", "work", and "worker" were referenced in victorian Scientific Life Writing. The goal is to determine whether references to work and play make up a larger proportion of the corpus at the end of the century than at the beginning, and to visualize this in such a way that scrolling over a point automatically produces a key words in context (randomly).
...
```{r,  eval=FALSE}
    ...
    library(readr)
    library(plotly)
    SciLifelocation <- paste0(getwd())
    SciLifedoclocation <- paste0(SciLifelocation,"/Corpora/SciLife")
    ...
    SciLifestemsearchedtermlist <- c(AmusementTerms,PlayTerms,RecreationTerms,LaborTerms,ToilTerms,WorkTerms)
    SciLifeoutputlocation <- paste0(getwd(),"/WordFlagDataFrames")
    SciLifeWordFlagdfPath <- paste0(SciLifeoutputlocation,"/","SciLifeWordFlagdf.txt")
    SciLifeDocumentSize <- 120676439
```
          ...
          for (j in 1:length(stemltoken)) {
              if (SciLifestemsearchedterm == stemltoken[j]) {
                ...
                #Determining Category
                  if(SciLifestemsearchedterm %in% AmusementTerms) {temprow[1,9] <- "Amusement"}
                  if(SciLifestemsearchedterm %in% PlayTerms) {temprow[1,9] <- "Play"}
                  if(SciLifestemsearchedterm %in% RecreationTerms) {temprow[1,9] <- "Recreation"}
                  if(SciLifestemsearchedterm %in% LaborTerms) {temprow[1,9] <- "Labor"}
                  if(SciLifestemsearchedterm %in% ToilTerms) {temprow[1,9] <- "Toil"}
                  if(SciLifestemsearchedterm %in% WorkTerms) {temprow[1,9] <- "Work"}
...
We can then add up the values in SciLifeWordFlagdf to make a table of the frequency of play and work terms for each text: SciLifeFreqmat.Again, it's important to do it this way because it lets us assign a random KWIC for later.
...
```{r,  eval=FALSE}
  # Adding values from SciLifeWordFlagdf together to get a matrix of normalized frequencies for each category, as SciLifeFreqmat
    ...
    for (i in 1:length(files)) {
      ...
      tempdf <- SciLifeWordFlagdf[grep(TempTextName,SciLifeWordFlagdf$Text),]
...
# Visualizing SciLifeFreqdf BY DATE
      p <- ggplot(SciLifeFreqdf, aes(y = as.numeric(as.character(Normalized_Freq)), x = as.numeric(substr(Date,1,4)), color = Category, label = Sample_KWIC))
...
Once we've verified that the data within texts looks right, we can do the same, but per decade, by creating a new dataframe: SciLifeFreqDecadedf.

```{r,  eval=FALSE}
  # Adding values from SciLifeFreqdf together to get a matrix of normalized frequencies for each decade, as SciLifeFreqDecadedf

      SciLifeFreqDecadeMat <- matrix(,ncol=6,nrow=1)
      
    Decades <- c(180, 181, 182, 183, 184, 185, 186, 187, 188, 189)
    for (z in 1:length(Categories)) { 
      tempdf <- SciLifeFreqdf[grep(Categories[z],SciLifeFreqdf$Category),]
      ...
      for (i in 1:length(Decades)) {
        ...
        SciLifeFreqDecadeMat <- rbind(SciLifeFreqDecadeMat,temprows)
      }
    }
    SciLifeFreqDecadeMat <- SciLifeFreqDecadeMat[-1,]
    SciLifeFreqDecadedf <- as.data.frame(SciLifeFreqDecadeMat)
    SciLifeFreqDecadedf
...
#set a category.
z=6
tempdf <- SciLifeFreqDecadedf[grep(Categories[z],SciLifeFreqDecadedf$Category),]
